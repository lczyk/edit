//! Unix terminal i/o.
//!
//! lifted from `edit::sys::unix` -- this module owns the raw-mode toggle, the
//! sigwinch-driven resize injection, and the polling stdin reader. nothing
//! filesystem- or icu-related lives here; that stayed in `edit::sys`.
...
const GIBI: usize = 1024 * 1024 * 1024;
...
/// query the terminal's current size via TIOCGWINSZ. returns `(cols, rows)`.
/// retries briefly because some terminal emulators answer slowly on startup.
/// falls back to (80, 24) if the ioctl never succeeds.
